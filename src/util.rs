...
pub use read_line_cb::ReadLineCb;
...
// unused_mut,unused_imports,non_upper_case_globals,clippy::assign_op_pattern,
// clippy::indexing_slicing,clippy::same_name_method,
// clippy::iter_without_into_iter,)]         const _: () = {
// 		impl<'gc> piccolo::FromValue<'gc> for $Name {
// 			fn from_value(_: piccolo::Context<'gc>, value: piccolo::Value<'gc>) ->
// Result<Self, piccolo::TypeError> { 				match value {
// 					piccolo::Value::UserData(ud) => {
// 						let k = *ud.downcast_static::<$Name>().map_err(|_| {
// 							piccolo::TypeError { expected: stringify!($Name), found:
// value.type_name() } 						})?;
//
// 						Ok(k)
// 					}
// 					_ => Err(piccolo::TypeError { expected: stringify!($Name), found:
// value.type_name() }), 				}
// 			}
// 		}
//
...
//             }
//
//             pub fn metatable<'gc>(ctx: piccolo::Context<'gc>) ->
// anyhow::Result<piccolo::Value<'gc>> { 				use piccolo::IntoValue;
//
